...
import { createEmpWithPassword, updateEmp, deleteEmp } from '../../lib/api/employee';
...
const initEmp = { number: "", name: "", joining_date: "", authority: appAuthorities[0].id, systempwd: "" };
...
const EmpNewPage = () => {
  ...
  const handleConfirmOK = async (dummy) => {
    ...
    try {
      const res = await createEmpWithPassword({ number: emp.number, name: emp.name, joining_date: emp.joining_date, authority: emp.authority, systempwd: emp.systempwd });
      if (res.data.status === 500) {
        setMessage({kbn: "error", msg: "社員情報登録エラー(500)"});
      } else if (res.data.status === 400) {
        setMessage({kbn: "error", msg: "システムパスワードが誤っています。"});
      } else {
        // 社員登録が正常だった場合
        try {
          // Devise SignUp
